...
function PerformancePage({ profile }: any) {
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{managerPerformances.length}명</div>
              <p className="text-xs text-muted-foreground mt-1">활동 매니저</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{totalStats.sales.toLocaleString()}원</div>
              <p className="text-xs text-muted-foreground mt-1">그린피 합계</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">수수료 합계</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">조인 판매 횟수</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="manager" className="space-y-4">
          ...
          <TabsContent value="manager">
            <Card>
              ...
              <CardContent>
                {loading ? (
                  <div className="flex items-center justify-center h-64">로딩 중...</div>
                      ...
                      <TableBody>
                        {managerPerformances.map((mp, index) => (
                          <TableRow key={mp.manager_id}>
                            ...
                            <TableCell className="text-right">{mp.total_count}건</TableCell>
                            <TableCell className="text-right whitespace-nowrap">
                              {Math.round(mp.total_sales / mp.total_count).toLocaleString()}원
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                      ...
                      <TableBody>
                        {golfClubPerformances.map((gp, index) => (
                          <TableRow key={gp.golf_club_name}>
                            <TableCell className="font-medium">{index + 1}</TableCell>
                            <TableCell className="font-medium">{gp.golf_club_name}</TableCell>
                            <TableCell className="text-right whitespace-nowrap">
                              {gp.total_sales.toLocaleString()}원
                            </TableCell>
                            <TableCell className="text-right">{gp.total_count}건</TableCell>
                            <TableCell className="text-right whitespace-nowrap">
                              {Math.round(gp.total_sales / gp.total_count).toLocaleString()}원
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
