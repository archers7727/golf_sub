...
function DepositPage({ profile }: any) {
  ...
  const updateStatus = async (id: string, newStatus: string, personName: string) => {
    const supabase = createClient()
              ...
              <TableRow key={item.id}>
                <TableCell className="whitespace-nowrap">
                  {item.course_times?.reserved_time
                    ? format(new Date(item.course_times.reserved_time), 'MM-dd (E)', { locale: ko })
                    : '-'}
                </TableCell>
                <TableCell className="whitespace-nowrap">
                ...
                <TableCell className="whitespace-nowrap">{item.phone_number}</TableCell>
                ...
                {showActions && (
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      {(item.status === '입금확인전' || item.status === '입금확인중') && (
                        <>
                          <Button
                            size="sm"
                            variant="default"
                            onClick={() => updateStatus(item.id, '입금완료', item.name)}
                          >
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => updateStatus(item.id, '입금확인중', item.name)}
                          >
                      ...
                      {item.status === '환불확인중' && (
                        <Button
                          size="sm"
                          variant="destructive"
                          onClick={() => updateStatus(item.id, '환불완료', item.name)}
                        >
    ...
    <DashboardLayout profile={profile}>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">입금 및 환불 상태를 관리합니다</p>
        </div>
...
export default withAuth(DepositPage, { requireAdmin: true })
