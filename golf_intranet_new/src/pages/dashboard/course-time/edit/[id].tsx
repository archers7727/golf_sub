...
function EditCourseTimePage({ user, profile }: any) {
  ...
  const { updateCourseTime, courseTimes, fetchCourseTimes } = useCourseTimeStore()
  ...
  // 데이터 로드
  useEffect(() => {
    if (id && courseTimes.length === 0) {
      fetchCourseTimes()
    }
  }, [id, courseTimes, fetchCourseTimes])

  useEffect(() => {
    if (id && courseTimes.length > 0) {
      const existingTime = courseTimes.find((t) => t.id === id)
      if (existingTime) {
        form.reset({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    setLoading(true)
    try {
      await updateCourseTime(id as string, values)
      toast.success('수정 완료', {
  ...
  return (
    <DashboardLayout profile={profile}>
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">타임 수정</h1>
          <p className="text-muted-foreground mt-1">타임 정보를 수정합니다</p>
        </div>
